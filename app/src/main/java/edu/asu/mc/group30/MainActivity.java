...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    public void Train(View view) throws IOException
    {
        classify();
        ...
        try {
            svm_model model = t.run(argument);
            TextView nu = (TextView)findViewById(R.id.nu);
            nu.setText("Nu - upper bound of training error rate:"+new DecimalFormat("##.##").format(model.param.nu));
            TextView obj = (TextView)findViewById(R.id.label);
            obj.setText("Labels:"+ Arrays.toString(model.label));
            TextView nbsv = (TextView)findViewById(R.id.rho);
            nbsv.setText("Rho - Bias term in decision function:"+new DecimalFormat("##.##").format(model.rho[0])
                    +", " + new DecimalFormat("##.##").format(model.rho[1])
                    +", " +new DecimalFormat("##.##").format(model.rho[2]));
            TextView nsv = (TextView)findViewById(R.id.nsv);
            nsv.setText("nSV - number of support vectors:"+Arrays.toString(model.nSV));
            toast = Toast.makeText(getApplicationContext(), "Model Built", duration);
            toast.show();
        }
        catch(Exception e)
        {
    ...
    public void Test(View view)
    {
        ...
        try {
            ...
            text.setText("Accuracy: " +new DecimalFormat("##.##").format(accuracy) + "%");
        }
        catch(Exception e)
        {
        ...
        protected  Void doInBackground(SensorEvent... my_events) {
            if (start_time != -1) {
                ...
                elapsedSeconds = total_time / 1000;
                {
                    ...
                    String ival = new DecimalFormat("##.##").format(x);
                    String jval = new DecimalFormat("##.##").format(y);
                    String kval = new DecimalFormat("##.##").format(z);
                    System.out.println("zzzzzzz ["+ ival + "," + jval + "," + kval +"]");
                    first_entry = false;
                    entry_count ++;
                    //}
